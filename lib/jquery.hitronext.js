...
function toggleLoading() {
  ...
  if ($('#alertType').text() == str) {
    $('#alertType')
        .parent()
        .append("<img src='img/loading.gif' width='20px' height='20px' />");
  } else {
...
$.extend({
  hitron : {
    ...
    checkLanAndSub : function(lanIp, submask) {
      ...
      var tmpM = eval(maskIp[3]) if (tmpM != 0) {
    ...
    checkIpRangeOverlap : function(beginA, endA, beginB, endB) {
      ...
      if ((beginBValue < beginAValue && endBValue < beginAValue) ||
          (beginBValue > endAValue && endBValue > endAValue)) {
        return false;
      } else {
        return true;
      }
    ...
    checkIpv6Address : function(ipv6) {
      var regex =
          /^([\da-fA-F]{1,4}:){7}[\da-fA-F]{1,4}$|^:((:[\da-fA-F]{1,4}){1,6}|:)$|^[\da-fA-F]{1,4}:((:[\da-fA-F]{1,4}){1,5}|:)$|^([\da-fA-F]{1,4}:){2}((:[\da-fA-F]{1,4}){1,4}|:)$|^([\da-fA-F]{1,4}:){3}((:[\da-fA-F]{1,4}){1,3}|:)$|^([\da-fA-F]{1,4}:){4}((:[\da-fA-F]{1,4}){1,2}|:)$|^([\da-fA-F]{1,4}:){5}:([\da-fA-F]{1,4})?$|^([\da-fA-F]{1,4}:){6}:$/;
      if (regex.test(ipv6))
        return true;
      else
        return false;
